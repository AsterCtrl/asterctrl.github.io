...
芯片能力由 BSP 提供，板卡资源由 target profile 提供，系统接线由 hardware profile
...
aster config validate deployments/production.yaml
```
